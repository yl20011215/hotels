...
function renderTable(){
  ...
  for (const h of hotels){
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="actions">
          <button class="linkbtn iconbtn" title="${shown ? "Hide Rooms" : "Show Rooms"}" aria-label="${shown ? "Hide Rooms" : "Show Rooms"}" data-toggle="${h.id}">
            ${shown ? "🙈" : "👁️"}
          </button>
          <button class="linkbtn iconbtn" title="Edit" aria-label="Edit" data-edit="${h.id}">✏️</button>
          <button class="linkbtn danger iconbtn" title="Delete" aria-label="Delete" data-del="${h.id}">🗑️</button>
        </div>
      </td>
    `;
    tbody.appendChild(tr);
    ...
    if (shown){
      for (const r of (h.rooms || [])){
        ...
        const rThumb = rHasPhotos
          ...
          : `<span class="thumb" title="No photos">📷</span>`;
        ...
        sub.innerHTML = `
          <td>${rThumb}</td>
          <td class="indent">
            <div class="roomTypeCell"><span class="roomArrow">↳</span><span class="roomTypeName">${escapeHtml(r.type)}</span></div>
            <!-- REMOVED: the photo-count pill under room name -->
          </td>
          ...
          <td>
            <div class="actions">
              <button class="linkbtn danger iconbtn" title="Delete Room" aria-label="Delete Room" data-del-room="${h.id}::${r.id}">🗑️</button>
            </div>
          </td>
        `;
...
// ---------- Bindings ----------
// Mobile UX: press Enter on hotel name to jump to next field
$("hotelName").addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    e.preventDefault();
    $("roomType").focus();
  }
